        ...
        <div class="w-full px-3 sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg border border-gray-200">
                <div class="p-6 text-gray-900">
                    ...
                    <!-- Form -->
                    <form action="{{ route('projects.tools.process-boq-selection', $project) }}" method="POST" id="boqSelectionForm">
                        @csrf
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    @foreach($sectionsWithSelectableDetails as $section)
        @foreach($section->details->where('harga_satuan', '>', 0) as $detail)
            ...
            if (isChecked) {
                ...
                if (maxUnit) {
                    ...
                    if (maxCount < checkboxes.length) {
                        showUnitWarning(`Choosen ${maxCount} items with units "${maxUnit}". ${checkboxes.length - maxCount} items with different units were not selected.`);
                    }
                }
            } else {
                // Uncheck all
