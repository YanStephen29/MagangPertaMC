    ...
    <div class="col-12">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">Tambah Tool</li>
            </ol>
        </nav>
    </div>
</div>
    ...
    <div class="col-md-10">
        <div class="card">
            <div class="card-header card-header-primary">
                <h5 class="mb-0">
                    <i class="fas fa-plus me-2"></i>Tambah Tool Baru untuk Event: {{ $event->title }}
                </h5>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('tools.store', $event->{'no_I/O'}) }}">
                    @csrf
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('Bidang_kodeGl') is-invalid @enderror" 
                                    ...
                                    @foreach($bidangs as $bidang)
                                        <option value="{{ $bidang->kodeGl }}" {{ old('Bidang_kodeGl') == $bidang->kodeGl ? 'selected' : '' }}>
                                            {{ $bidang->nama_Bidang }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('Document_no_request') is-invalid @enderror" 
                                    ...
                                    @foreach($documents as $document)
                                        <option value="{{ $document->no_request }}" {{ old('Document_no_request') == $document->no_request ? 'selected' : '' }}>
                                            {{ $document->no_request }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="d-flex justify-content-between">
                        ...
                        <button type="submit" class="btn btn-primary">
                            <i class="fas fa-save me-1"></i>Simpan Tool
                        </button>
                    </div>
                </form>
            </div>
